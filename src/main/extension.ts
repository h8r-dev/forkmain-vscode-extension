...
export let appTreeView:
	| vscode.TreeView<BaseNocalhostNode>
	| null
	| undefined;
...
export async function activate(context: vscode.ExtensionContext) {
	...
	let isSetVisible =
		host.getGlobalState(TMP_WORKLOAD_PATH) ===
	...
	if (
		state.getData('app')?.action &&
		state.getData('app')?.action !== 'open'
	) {
		launchDevSpace();
	}
...
function bindEvent() {
	...
	messageBus.on('devStart', value => {
		host.log(
			'[Catch devStart] Message Bus catch devStart event.',
			true
		);
