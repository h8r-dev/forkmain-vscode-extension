	...
	async execCommand(
		...
		if (action === "stop") {
			...
			host.showProgressing(
				...
				async () => {
					let newLocalCluster =
						await LocalCluster.appendLocalClusterByKubeConfig(
							kubeconfig
						);
	...
	private handleActions(
		...
		switch (action) {
			case "run":
				...
				break;
			case "open":
				host.log("Entering open in vscode mode.", true);
				vscode.commands.executeCommand(OPEN, targetWorkloadNode, {
					isAutoMode: true,
				}); // Enter dev mode.
				break;
			case "debug":
				...
				host.showInformationMessage(
					"Waiting for entering debug mode..."
				);
